...
def silhouette_calculator(X, cluster):
    ...
    # dictionary 형태에서 value들을 추출해서 리스트 형태로 변환해 줍니다.
...
def main():
    ...
    silhouette = silhouette_calculator(X, Kmeans.cluster) # 모델객체.labels_
    
    # 군집의 개수와 silhouette 값을 출력합니다. 
    print((Kmeans.cluster.values()))
    print('K = {}, silhouette 값: {}'.format(len(set(Kmeans.cluster.values())), silhouette))
